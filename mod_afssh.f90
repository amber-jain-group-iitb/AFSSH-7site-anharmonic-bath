...
!stop
  ...
  allocate(Hamil_site(nbasis,nbasis),Hamil_site1(nbasis,nbasis),Hamil_diab(nbasis,nbasis),delH_dels(nbasis,nbasis,nclass),delH_dels_ad(nquant,nquant,nclass))
...
!stop
  do i=1,N_traj
    traj_num=i
Hamil_site=Hamil_site1

    call init_cond
    call evolve(1,100000)

    call setup_parameters
    call evolve(100001,100000+nsteps)
    call average_end
  enddo
  call write_average
...
subroutine write_average
  ...
  do i=1,(nsteps+100000)/nstep_avg
    pop_el=0.d0
   ! do i1=1,2
   !   j=(i1-1)*nb_vib
   !   do k=1,nb_vib
...
subroutine setup_parameters
  ...
  real*8 c_0,c_e
  real*8 omg_max,delw,w_c

  mass=1836.d0*au2kg

 omg_max=10d0*omg_B
 w_c=omg_max/10d0
delw=omg_max/real(nclass/7)



 do i=1,nclass/7
    omg(i)=(i)*delw
!    omg(i)=-w_c*log((i-0.5d0)/float(nclass-1))
    ck(i)=dsqrt(2*spectral(omg(i))*mass(i)*omg(i)*delw/pi)
!    ck(i)=omg(i)*dsqrt(2d0*gamma_B*(mass(1)**2)*w_c/(float(nclass-1)*pi))
!write(1,*) omg(i)/(2d0*pi*clight),spectral(omg(i))
  enddo

omg(1+nclass/7:2*nclass/7)=omg(1:nclass/7)
omg(1+2*nclass/7:3*nclass/7)=omg(1:nclass/7)
omg(1+3*nclass/7:4*nclass/7)=omg(1:nclass/7)
omg(1+4*nclass/7:5*nclass/7)=omg(1:nclass/7)
omg(1+5*nclass/7:6*nclass/7)=omg(1:nclass/7)
omg(1+6*nclass/7:nclass)=omg(1:nclass/7)
!write(*,*) omg/(2d0*pi*clight)

ck(1+nclass/7:2*nclass/7)=ck(1:nclass/7)
ck(1+2*nclass/7:3*nclass/7)=ck(1:nclass/7)
ck(1+3*nclass/7:4*nclass/7)=ck(1:nclass/7)
ck(1+4*nclass/7:5*nclass/7)=ck(1:nclass/7)
ck(1+5*nclass/7:6*nclass/7)=ck(1:nclass/7)
ck(1+6*nclass/7:nclass)=ck(1:nclass/7)

!write(*,*)sum(ck(1:nclass/3)**2/(mass(1:nclass/3)*omg(1:nclass/3)**2))/wave_to_j
!write(*,*) omg_B!ck,mass,omg 
!stop

 dk(1:nclass/7)=(hbar*omg(1:nclass/7)*2d0)/2d0
 alpha(1:nclass/7)=dsqrt((mass(1)*omg(1:nclass/7))/(hbar*2d0))

dk(1+nclass/7:2*nclass/7)=dk(1:nclass/7)
dk(1+2*nclass/7:3*nclass/7)=dk(1:nclass/7)
dk(1+3*nclass/7:4*nclass/7)=dk(1:nclass/7)
dk(1+4*nclass/7:5*nclass/7)=dk(1:nclass/7)
dk(1+5*nclass/7:6*nclass/7)=dk(1:nclass/7)
dk(1+6*nclass/7:nclass)=dk(1:nclass/7)

alpha(1+nclass/7:2*nclass/7)=alpha(1:nclass/7)
alpha(1+2*nclass/7:3*nclass/7)=alpha(1:nclass/7)
alpha(1+3*nclass/7:4*nclass/7)=alpha(1:nclass/7)
alpha(1+4*nclass/7:5*nclass/7)=alpha(1:nclass/7)
alpha(1+5*nclass/7:6*nclass/7)=alpha(1:nclass/7)
alpha(1+6*nclass/7:nclass)=alpha(1:nclass/7)


!  g_coup=dsqrt(lambda_B*mass(1)*omg_B**2/2.d0)

  Hamil_site1=0.d0
  Hamil_site1(1,1)=410.d0
  Hamil_site1(2,2)=530.d0
  Hamil_site1(3,3)=210.d0
  Hamil_site1(4,4)=320.d0
  Hamil_site1(5,5)=480.d0
  Hamil_site1(6,6)=630.d0
  Hamil_site1(7,7)=440.d0

  Hamil_site1(1,2)=0.0001d0; Hamil_site1(2,1)=Hamil_site1(1,2)
  Hamil_site1(2,3)=0.0001d0; Hamil_site1(3,2)=Hamil_site1(2,3)
  Hamil_site1(3,4)=0.0001d0; Hamil_site1(4,3)=Hamil_site1(3,4)
  Hamil_site1(4,5)=0.0001d0; Hamil_site1(5,4)=Hamil_site1(4,5)
  Hamil_site1(5,6)=0.0001d0; Hamil_site1(6,5)=Hamil_site1(5,6)
  Hamil_site1(6,7)=0.0001d0; Hamil_site1(7,6)=Hamil_site1(6,7)

  Hamil_site1(1,3)=0.0001d0; Hamil_site1(3,1)=Hamil_site1(1,3)
  Hamil_site1(1,4)=0.0001d0; Hamil_site1(4,1)=Hamil_site1(1,4)
  Hamil_site1(1,5)=0.0001d0; Hamil_site1(5,1)=Hamil_site1(1,5)
  Hamil_site1(1,6)=0.0001d0; Hamil_site1(6,1)=Hamil_site1(1,6)
  Hamil_site1(1,7)=0.0001d0; Hamil_site1(7,1)=Hamil_site1(1,7)
  Hamil_site1(2,4)=0.0001d0; Hamil_site1(4,2)=Hamil_site1(2,4)
  Hamil_site1(2,5)=0.0001d0; Hamil_site1(5,2)=Hamil_site1(2,5)
  Hamil_site1(2,6)=0.0001d0; Hamil_site1(6,2)=Hamil_site1(2,6)
  Hamil_site1(2,7)=0.0001d0; Hamil_site1(7,2)=Hamil_site1(2,7)
  Hamil_site1(3,5)=0.0001d0; Hamil_site1(5,3)=Hamil_site1(3,5)
  Hamil_site1(3,6)=0.0001d0; Hamil_site1(6,3)=Hamil_site1(3,6)
  Hamil_site1(3,7)=0.0001d0; Hamil_site1(7,3)=Hamil_site1(3,7)
  Hamil_site1(4,6)=0.0001d0; Hamil_site1(6,4)=Hamil_site1(4,6)
  Hamil_site1(4,7)=0.0001d0; Hamil_site1(7,4)=Hamil_site1(7,4)
  Hamil_site1(5,7)=0.0001d0; Hamil_site1(7,5)=Hamil_site1(7,5)
  
  Hamil_site1=Hamil_site1*wave_to_J

  Hamil_site=0.d0
  Hamil_site(1,1)=410.d0
  Hamil_site(2,2)=530.d0
  Hamil_site(3,3)=210.d0
  Hamil_site(4,4)=320.d0
  Hamil_site(5,5)=480.d0
  Hamil_site(6,6)=630.d0
  Hamil_site(7,7)=440.d0

  Hamil_site(1,2)=-87.7d0; Hamil_site(2,1)=Hamil_site(1,2)
  Hamil_site(2,3)=30.8d0; Hamil_site(3,2)=Hamil_site(2,3)
  Hamil_site(3,4)=-53.5d0; Hamil_site(4,3)=Hamil_site(3,4)
  Hamil_site(4,5)=-70.7d0; Hamil_site(5,4)=Hamil_site(4,5)
  Hamil_site(5,6)=81.1d0; Hamil_site(6,5)=Hamil_site(5,6)
  Hamil_site(6,7)=39.7d0; Hamil_site(7,6)=Hamil_site(6,7)

  Hamil_site(1,3)=5.5d0; Hamil_site(3,1)=Hamil_site(1,3)
  Hamil_site(1,4)=-5.9d0; Hamil_site(4,1)=Hamil_site(1,4)
  Hamil_site(1,5)=6.7d0; Hamil_site(5,1)=Hamil_site(1,5)
  Hamil_site(1,6)=-13.7d0; Hamil_site(6,1)=Hamil_site(1,6)
  Hamil_site(1,7)=-9.9d0; Hamil_site(7,1)=Hamil_site(1,7)
  Hamil_site(2,4)=8.2d0; Hamil_site(4,2)=Hamil_site(2,4)
  Hamil_site(2,5)=0.7d0; Hamil_site(5,2)=Hamil_site(2,5)
  Hamil_site(2,6)=11.8d0; Hamil_site(6,2)=Hamil_site(2,6)
  Hamil_site(2,7)=4.3d0; Hamil_site(7,2)=Hamil_site(2,7)
  Hamil_site(3,5)=-2.2d0; Hamil_site(5,3)=Hamil_site(3,5)
  Hamil_site(3,6)=-9.6d0; Hamil_site(6,3)=Hamil_site(3,6)
  Hamil_site(3,7)=6d0; Hamil_site(7,3)=Hamil_site(3,7)
  Hamil_site(4,6)=-17d0; Hamil_site(6,4)=Hamil_site(4,6)
  Hamil_site(4,7)=-63.3d0; Hamil_site(7,4)=Hamil_site(7,4)
  Hamil_site(5,7)=-1.3d0; Hamil_site(7,5)=Hamil_site(7,5)
...
subroutine compute_potential(H_diab,delV_dels)
  ...
  integer i,j,k,n,m,l,p
  real*8 h1,h2,h3,dh1(nclass)
  ...
  H1=sum(Dk*((dexp(-2d0*alpha*x))-2d0*dexp(-alpha*x)))
  dH1=2d0*Dk*(1-dexp(-alpha*x))*alpha*dexp(-alpha*x)

    H_diab(1,1)=H_diab(1,1)+H1-sum(ck(1:n)*(1-dexp(-alpha(1:n)*x(1:n)))/alpha(1:n))
    H_diab(2,2)=H_diab(2,2)+H1-sum(ck(n+1:m)*(1-dexp(-alpha(n+1:m)*x(n+1:m)))/alpha(n+1:m))
    H_diab(3,3)=H_diab(3,3)+H1-sum(ck(m+1:l)*(1-dexp(-alpha(m+1:l)*x(m+1:l)))/alpha(m+1:l))
    H_diab(4,4)=H_diab(4,4)+H1-sum(ck(l+1:k)*(1-dexp(-alpha(l+1:k)*x(l+1:k)))/alpha(l+1:k))
    H_diab(5,5)=H_diab(5,5)+H1-sum(ck(k+1:j)*(1-dexp(-alpha(k+1:j)*x(k+1:j)))/alpha(k+1:j))
    H_diab(6,6)=H_diab(6,6)+H1-sum(ck(j+1:p)*(1-dexp(-alpha(j+1:p)*x(j+1:p)))/alpha(j+1:p))
    H_diab(7,7)=H_diab(7,7)+H1-sum(ck(p+1:nclass)*(1-dexp(-alpha(p+1:nclass)*x(p+1:nclass)))/alpha(p+1:nclass))

 do i=1,nquant
  delV_dels(i,i,:)=delV_dels(i,i,:)+dh1(:)
  enddo

  delv_dels(1,1,1:n)=delv_dels(1,1,1:n)-ck(1:n)*dexp(-alpha(1:n)*x(1:n))
  delv_dels(2,2,n+1:m)=delv_dels(2,2,n+1:m)-ck(n+1:m)*dexp(-alpha(n+1:m)*x(n+1:m))
  delv_dels(3,3,m+1:l)=delv_dels(3,3,m+1:l)-ck(m+1:l)*dexp(-alpha(m+1:l)*x(m+1:l))
  delv_dels(4,4,l+1:k)=delv_dels(4,4,l+1:k)-ck(l+1:k)*dexp(-alpha(l+1:k)*x(l+1:k))
  delv_dels(5,5,k+1:j)=delv_dels(5,5,k+1:j)-ck(k+1:j)*dexp(-alpha(k+1:j)*x(k+1:j))
  delv_dels(6,6,j+1:p)=delv_dels(6,6,j+1:p)-ck(j+1:p)*dexp(-alpha(j+1:p)*x(j+1:p))
  delv_dels(7,7,p+1:nclass)=delv_dels(7,7,p+1:nclass)-ck(p+1:nclass)*dexp(-alpha(p+1:nclass)*x(p+1:nclass))
